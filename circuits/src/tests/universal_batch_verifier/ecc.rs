...
use halo2_base::{
    ...
    halo2_proofs::{
        ...
        dev::MockProver,
        halo2curves::{
            bn256::{Fq, Fq2, Fr, G2Affine},
            group::cofactor::{CofactorCurveAffine, CofactorGroup},
        },
    ...
    R: RngCore + ?Sized,
{
    loop {
        ...
        if let Some(y) = Option::<Fq2>::from(y) {
            ...
            let p = p_affine.to_curve();
            let p_affine_clear = G2Affine::from(p.clear_cofactor());
            if p_affine_clear != p_affine {
                return p_affine;
            }
        }
    }
...
fn test_g2_membership_for_subgroup_check(subgroup_check: SubgroupCheck) {
    ...
    match subgroup_check {
        SubgroupCheck::None => {
            is_satisfied.expect(
            "Verification shouldn't fail if when the subgroup isn't checked",
        );
        }
        _ => {
            is_satisfied.expect_err(
                "Verification should fail if when the subgroup isn't checked",
            );
        }
    }
}
