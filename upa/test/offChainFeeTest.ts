...
import { OffChainSubmission } from "../src/sdk";
...
type DeployAndSubmitResult = {
  ...
  upa: UpaInstance;
  upaClient: UpaClient;
  submission_1: OffChainSubmission;
  submission_2: OffChainSubmission;
  submission_3: OffChainSubmission;
  cid_a: string;
...
async function deployAndSubmit(): Promise<DeployAndSubmitResult> {
  ...
  const submission_1 =
    OffChainSubmission.fromCircuitIdsProofsAndInputs(cidProofAndInputs_1);
  const submission_2 =
    OffChainSubmission.fromCircuitIdsProofsAndInputs(cidProofAndInputs_2);
  const submission_3 =
    OffChainSubmission.fromCircuitIdsProofsAndInputs(cidProofAndInputs_3);
...
describe.only("Off-chain submission fees", async () => {
  it("Sign a message and verify the signature", async () => {
    ...
    //const chainId = (await worker.provider!.getNetwork()).chainId;
    ...
    const contractRecoveredSigner = await upaOffChainFee.recoverSigner(
      SignedData,
      signedRequest.signature
    );
