...
export async function setupEnvAndRepeatCommand() {
  ...
  // Write upa_deployment.hardhat.config.js using the CHAIN_ENDPOINT env var.
  // We make sure to set this CHAIN_ENDPOINT as the default network, as
  // OpenZeppelin checks that the contract was deployed using this default.
  const chainEndpoint = process.env.CHAIN_ENDPOINT;
  if (!chainEndpoint) {
    throw new Error("CHAIN_ENDPOINT environment variable is not set");
  }
